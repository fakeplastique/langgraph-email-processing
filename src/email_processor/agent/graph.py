...
def build_graph(blob_store: BlobStore, llm: BaseChatModel, llm_retry_kwargs: dict | None = None):
    ...
    classify_invoke = llm_retry(llm.with_structured_output(ClassificationOutput).invoke)
    summarize_invoke = llm_retry(llm.with_structured_output(SummaryOutput).invoke)
